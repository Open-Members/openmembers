...
export interface WelcomeWithPasswordVars extends EmailRenderContext {
...
export interface WelcomeWithPasswordContent {
  subject: string;
  heading: string;
  intro: string;
...
export const WELCOME_DEFAULT_CONTENT: WelcomeWithPasswordContent =
  EMAIL_TEMPLATE_DEFAULTS.welcome_with_password.en;

function buildVars(vars: WelcomeWithPasswordVars) {
...
interface RenderArgs extends WelcomeWithPasswordVars {
  content: WelcomeWithPasswordContent;
}

export function WelcomeWithPasswordEmail(args: RenderArgs) {
  ...
  return (
    <EmailLayout
      preview={renderEmailFrameText(locale, frame.previews.welcomeWithPassword, { siteName: args.siteName })}
...
export async function renderWelcomeWithPassword(
  vars: WelcomeWithPasswordVars,
  content?: WelcomeWithPasswordContent,
  ...
  const resolvedContent = applyVarsToContent(
    content ?? EMAIL_TEMPLATE_DEFAULTS.welcome_with_password[locale],
    resolvedVars,
  );

  const element = (
    <WelcomeWithPasswordEmail {...vars} locale={locale} now={now} content={resolvedContent} />
  );

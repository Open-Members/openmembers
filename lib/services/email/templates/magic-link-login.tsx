...
export interface MagicLinkLoginVars extends EmailRenderContext {
...
export interface MagicLinkLoginContent {
  subject: string;
  heading: string;
  intro: string;
  ctaLabel: string;
...
export const MAGIC_LINK_LOGIN_VAR_KEYS = [
  'firstName',
  'studentName',
  'studentEmail',
  'loginUrl',
...
export const MAGIC_LINK_LOGIN_DEFAULT_CONTENT: MagicLinkLoginContent =
  EMAIL_TEMPLATE_DEFAULTS.magic_link_login.en;

function buildVars(vars: MagicLinkLoginVars) {
...
interface RenderArgs extends MagicLinkLoginVars {
  content: MagicLinkLoginContent;
}

export function MagicLinkLoginEmail(args: RenderArgs) {
  ...
  return (
    <EmailLayout
      preview={renderEmailFrameText(locale, frame.previews.magicLinkLogin, { siteName: args.siteName })}
...
export async function renderMagicLinkLogin(
  vars: MagicLinkLoginVars,
  content?: MagicLinkLoginContent,
): Promise<{ subject: string; html: string; text: string }> {
  const locale = resolveEmailLocale(vars.locale);
  const now = vars.now ?? new Date();
  const resolvedVars = buildVars(vars);
  const resolvedContent = applyVarsToContent(content ?? EMAIL_TEMPLATE_DEFAULTS.magic_link_login[locale], resolvedVars);

  const element = <MagicLinkLoginEmail {...vars} locale={locale} now={now} content={resolvedContent} />;

...
export interface EmailConfirmationVars extends EmailRenderContext {
...
export interface EmailConfirmationContent {
  subject: string;
  heading: string;
  intro: string;
  ctaLabel: string;
...
export const EMAIL_CONFIRMATION_VAR_KEYS = [
  'firstName',
  'studentName',
  'studentEmail',
  'confirmUrl',
...
export const EMAIL_CONFIRMATION_DEFAULT_CONTENT: EmailConfirmationContent =
  EMAIL_TEMPLATE_DEFAULTS.email_confirmation.en;

function buildVars(vars: EmailConfirmationVars) {
...
interface RenderArgs extends EmailConfirmationVars {
  content: EmailConfirmationContent;
}

export function EmailConfirmationEmail(args: RenderArgs) {
  ...
  return (
    <EmailLayout
      preview={renderEmailFrameText(locale, frame.previews.emailConfirmation, { siteName: args.siteName })}
...
export async function renderEmailConfirmation(
  vars: EmailConfirmationVars,
  content?: EmailConfirmationContent,
): Promise<{ subject: string; html: string; text: string }> {
  const locale = resolveEmailLocale(vars.locale);
  const now = vars.now ?? new Date();
  const resolvedVars = buildVars(vars);
  const resolvedContent = applyVarsToContent(content ?? EMAIL_TEMPLATE_DEFAULTS.email_confirmation[locale], resolvedVars);

  const element = <EmailConfirmationEmail {...vars} locale={locale} now={now} content={resolvedContent} />;
